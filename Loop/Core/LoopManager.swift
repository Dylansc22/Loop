...
extension LoopManager {
        ...
        if isGridModeEnabled {
            ...
            if resolvedStartingAction.direction == .noSelection,
               ...
               !initialGridAction.direction.isNoOp {
                // Use the window's actual frame for the preview so it doesn't
                // snap to grid-quantized coordinates on trigger.
                let pb = resizeContext.paddedBounds
                let wf = window?.frame ?? resizeContext.cachedTargetFrame.raw
                let windowAction = WindowAction(
                    .custom,
                    keybind: [],
                    name: "autogenerated_grid_action",
                    unit: .percentage,
                    width: (wf.width / pb.width) * 100,
                    height: (wf.height / pb.height) * 100,
                    xPoint: ((wf.minX - pb.minX) / pb.width) * 100,
                    yPoint: ((wf.minY - pb.minY) / pb.height) * 100,
                    positionMode: .coordinates,
                    sizeMode: .custom
                )
                triggerActionID = windowAction.id
                resolvedStartingAction = windowAction
                resizeContext.setAction(to: windowAction, parent: nil)
            }
    ...
    private func closeLoop(forceClose: Bool) async {
        ...
        if !forceClose {
            let gridCancelled = triggerActionID != nil && resizeContext.action.id == triggerActionID
